...
/*  
 *  This variable should be placed in a non initialized RAM section in
 *  order to be valid upon soft-reset from application into bootloader.
 */
static dfu_ble_peer_data_t m_peer_data ; //__attribute__((section("NoInit"), zero_init));

/*
 *  CRC variable to ensure the integrity of the peer data provided.
 */
static uint16_t            m_peer_data_crc ; //__attribute__((section("NoInit"), zero_init));
...
#warning "building with ARM toolchain"
